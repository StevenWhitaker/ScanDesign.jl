...
Date Created: 2018-06-07
    ...
    scandesign(P0, costfun;
               algorithm = :G_MLSL, lb = -Inf, ub = Inf, stopval = -Inf,
               tol_rel = 0, tol_abs = 0, maxeval = 0, maxtime = 0,
               localalgorithm = :LD_SLSQP, verbosecount = 100)
...
- `costfun::Function = expectedcost`: Cost function to minimize, has calling
    signature `(cost [, grad]) = costfun(P, computegrad)` where `P` is the scan
    design and `computegrad` is a `Bool` that specifies whether to compute the
...
function createcostfunction(costfun, PfromPv, verbosecount)
  ...
  fn(Pv::Vector, gradv::Vector) = begin
    ...
    if length(gradv) > 0
      ...
      # I was going to explicitly check if gradxp is empty, but this will throw
      # an error already if gradxp is empty
      (cost, grad) = costfun(P, true)
      gradv[:] = P2Pv(grad) # This works because grad and P have the same shape
